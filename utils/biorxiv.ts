...
export class BiorxivService {
  private static readonly RSS_URL =
  ...
  static async fetchFeed(url: string): Promise<string | any> {
    ...
    if (cachedItem && now - cachedItem.timestamp < this.CACHE_TTL) {
      console.log("Using cached bioRxiv data for:", url);
    ...
    const isDateRangeUrl =
      url.includes(`${this.API_URL}/`) && url.includes("-");
  ...
  static parsePaper(item: any): Paper {
    ...
    return {
      title:
        item.title?.[0]?.replace(/<!\[CDATA\[(.*?)\]\]>/g, "$1")?.trim() ||
        "No title available",
      ...
      categories: [
        item["prism:section"] ? item["prism:section"][0] : "Unknown",
      ],
      ...
      guid: item["dc:identifier"]
        ? item["dc:identifier"][0].replace("doi:", "")
        : null,
  ...
  static async fetchPapersByDateRange(
    startDate: string,
